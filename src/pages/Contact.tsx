...
import { sendContactMessage } from '../store/product/productAsyncAction';
  ...
  const renderInput = useCallback(() => {
    return MESSAGE_FIELDS.map((field) => (
      <TextField
        ...
        error={
          formik.touched[field.name as MessageFieldName] &&
          Boolean(formik.errors[field.name as MessageFieldName])
        }
        helperText={
          formik.touched[field.name as MessageFieldName] &&
          formik.errors[field.name as MessageFieldName]
        }
        fullWidth
      ...
      <Grid size={7}>
        <WhitePaper>
          <form onSubmit={formik.handleSubmit} style={{ width: '100%' }}>
            ...
            >
              <Grid size={4}>{renderInput()[0]}</Grid>
              <Grid size={4}>{renderInput()[1]}</Grid>
              <Grid size={4}>{renderInput()[2]}</Grid>
              <Grid size={12}>{renderInput()[3]}</Grid>
              <Grid size={12}>
                <LoadingButton
                  type='submit'
                  variant='contained'
